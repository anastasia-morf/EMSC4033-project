...
#Define variables
    #Exception Handling 
    if "age" in data.keys() and "year" in data.keys(): #Define x-axis
        print("Selecting Age for the x-axis")
        year = data['age']
        plt.xlabel(data['ageUnits'])
    elif "age" in data.keys():
        print("Only age available - selecting age for the x-axis ")
        year = data['age']
        plt.xlabel(data['ageUnits'])
    elif "year" in data.keys():
        print("Only year available - selecting age for the x-axis")
        year = data['year']
        plt.xlabel(data['yearUnits'])
    else: 
        raise KeyError("No age or year available - cannot plot")
    
    values = np.sort(data['paleoData_values']) #Define x-axis
    
#Make plot     
    plt.plot(year,values)
    plt.title("Name: "+data['dataSetName']+", archive: "+data['archiveType'])
    plt.ylabel(data['paleoData_variableName'])
    #plt.gca().invert_xaxis() # Here, you can invert the x-axis. This is just a preference and is optional.
    #plt.gca().invert_yaxis() # Here, you can invert the y-axis. This is just a preference and is optional.
